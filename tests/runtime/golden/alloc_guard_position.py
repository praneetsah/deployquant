...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['buyspy', 'reb']}
WEIGHTS_REB = {'best': {'of': [{'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}


class AllocShapes(QCAlgorithm):
    """Alloc Shapes — ejected from the block editor. Runs the SAME
    ...
    def initialize(self):
        ...
        self._alloc_reb = Allocator(self, WEIGHTS_REB, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_buyspy(self):
        # rule: buyspy  (market_order on SPY)
        if not ((not (self.portfolio['SPY'].invested))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.2) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:buyspy")

    def _rule_reb(self):
        # rule: reb  (set_weights on SPY)
        if not (not self._guard_flag.get("position:reb")):
            return
        self._guard_flag["position:reb"] = True
        self._alloc_reb.rebalance(
            self.time.date(), tag="ir:reb", on_flat=self._on_flat)
