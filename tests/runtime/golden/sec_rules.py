...
class SecondBlocks(QCAlgorithm):
    ...
    def _rule_buy(self):
        # rule: buy  (market_order on AAA)
        if not ((not (self.portfolio['AAA'].invested))):
            return
        ref_price = self._bc.price('AAA')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.8) / ref_price)
        if qty > 0:
            self.market_order('AAA', 1 * qty, tag="ir:buy")

    def _rule_flat(self):
        # rule: flat  (liquidate on AAA)
        self._liquidate_all('AAA', "ir:flat")
