...
class IndicatorGate(QCAlgorithm):
    ...
    def _rule_enter(self):
        ...
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'price', 'field': 'close'}, 'SPY')) > (self._bc.indicator({'ind': 'sma', 'window': 50}, 'SPY')))))):
            return
    ...
    def _rule_exit(self):
        ...
        if not ((not (self._cmp(lambda: (self._bc.indicator({'ind': 'price', 'field': 'close'}, 'SPY')) > (self._bc.indicator({'ind': 'sma', 'window': 50}, 'SPY')))))):
            return
