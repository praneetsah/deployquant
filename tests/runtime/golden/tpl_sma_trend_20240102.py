...
PARAMS = {'FAST': 10, 'SLOW': 50, 'SIZE': 0.98}
SYMS = ['TQQQ']
UNIVERSE = ['TQQQ']
PRIMARY = 'TQQQ'
RULES_BY_SYMBOL = {'TQQQ': ['trend-entry', 'trend-exit']}


class SmaTrendFilter(QCAlgorithm):
    """SMA Trend Filter — ejected from the block editor. Runs the SAME
    ...
    def _rule_trend_entry(self):
        # rule: trend-entry  (market_order on TQQQ)
        if not ((((not (self.portfolio['TQQQ'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'sma', 'window': {'param': 'FAST'}}, 'TQQQ')) > (self._bc.indicator({'ind': 'sma', 'window': {'param': 'SLOW'}}, 'TQQQ')))))):
            return
        ref_price = self._bc.price('TQQQ')
        qty = int(float(self.portfolio.total_portfolio_value) * (self.SIZE) / ref_price)
        if qty > 0:
            self.market_order('TQQQ', 1 * qty, tag="ir:trend-entry")

    def _rule_trend_exit(self):
        # rule: trend-exit  (liquidate on TQQQ)
        if not (((self.portfolio['TQQQ'].invested) and (self._cmp(lambda: (self._bc.indicator({'ind': 'sma', 'window': {'param': 'FAST'}}, 'TQQQ')) < (self._bc.indicator({'ind': 'sma', 'window': {'param': 'SLOW'}}, 'TQQQ')))))):
            return
        self._liquidate_all('TQQQ', "ir:trend-exit")
    ...
    def _at_open(self):
        ...
        try:
            self._rule_trend_entry()
        except NotReady:
            pass
        try:
            self._rule_trend_exit()
        except NotReady:
            pass
