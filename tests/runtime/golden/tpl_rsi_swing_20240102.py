...
PARAMS = {'RSI_WINDOW': 14, 'BUY_BELOW': 30, 'SELL_ABOVE': 70, 'SIZE': 0.98}
SYMS = ['SPY']
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['buy-oversold', 'sell-overbought']}


class RsiBuyLowSellHigh(QCAlgorithm):
    """RSI Buy Low, Sell High — ejected from the block editor. Runs the SAME
    ...
    def _rule_buy_oversold(self):
        # rule: buy-oversold  (market_order on SPY)
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': {'param': 'RSI_WINDOW'}}, 'SPY')) < (self.BUY_BELOW))))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (self.SIZE) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:buy-oversold")

    def _rule_sell_overbought(self):
        # rule: sell-overbought  (liquidate on SPY)
        if not (((self.portfolio['SPY'].invested) and (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': {'param': 'RSI_WINDOW'}}, 'SPY')) > (self.SELL_ABOVE))))):
            return
        self._liquidate_all('SPY', "ir:sell-overbought")
    ...
    def _at_open(self):
        ...
        try:
            self._rule_buy_oversold()
        except NotReady:
            pass
        try:
            self._rule_sell_overbought()
        except NotReady:
            pass
