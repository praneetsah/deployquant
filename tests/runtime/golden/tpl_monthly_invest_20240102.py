...
PARAMS = {'MONTHLY_DOLLARS': 500}
SYMS = ['SPY']
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['monthly-buy']}


class MonthlyAutoInvest(QCAlgorithm):
    """Monthly Auto-Invest — ejected from the block editor. Runs the SAME
    ...
    def _rule_monthly_buy(self):
        # rule: monthly-buy  (market_order on SPY)
        if not self._is_first_of_month():
            return
        if not (self._guard_month.get('monthly-buy') != (self._current_day.year, self._current_day.month)):
            return
        if not (self._cmp(lambda: (self.portfolio.cash) > (50))):
            return
        ref_price = self._bc.price('SPY')
        qty = int((self.MONTHLY_DOLLARS) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:monthly-buy")
            self._guard_month['monthly-buy'] = (self._current_day.year, self._current_day.month)
