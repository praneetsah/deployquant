...
class AllocShapes(QCAlgorithm):
    ...
    def _rule_buy(self):
        # rule: buy  (market_order on SPY)
        if not self._is_first_of_week():
            return
        if not ((not (self.portfolio['SPY'].invested))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.8) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:buy")

    def _rule_tp(self):
        # rule: tp  (managed_target on SPY)
        if not (self._cmp(lambda: (self.portfolio['SPY'].quantity) > (0))):
            return
        # qty='0.5': the IR engine never reads managed_target['qty'] and always
        # exits the whole position, so this target is for all of it.
        qty = int(self.portfolio['SPY'].quantity)
        if qty <= 0:
            return
        limit_px = ((self._bc.entry_price('SPY')) * (1.02))
        self._place_or_update_target('SPY', qty, limit_px, tag="ir:tp")
