...
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['enter', 'moc']}


class SellShapes(QCAlgorithm):
    """Sell Shapes — ejected from the block editor. Runs the SAME
    ...
    def _rule_enter(self):
        ...
        if not self._is_first_of_week():
            return
        if not (not self._guard_flag.get("position:enter")):
            return
        if not ((not (self.portfolio['SPY'].invested))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.9) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:enter")
            self._guard_flag["position:enter"] = True

    def _rule_moc(self):
        # rule: moc  (at_close_order on SPY)
        if not (self._cmp(lambda: (self.portfolio['SPY'].quantity) > (0))):
            return
        qty = abs(int(self.portfolio['SPY'].quantity))
        if qty <= 0:
            return
        close_px = self._bc.price('SPY')
        limit_px = round(float(((self._bc.price('SPY')) * (1.1))), 2)
        if not (close_px >= limit_px):
            return                      # LOC expires unfilled
        self.market_order('SPY', -1 * qty, tag="ir:moc")
