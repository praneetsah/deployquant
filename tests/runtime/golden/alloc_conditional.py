...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['c']}
WEIGHTS_C = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_c = Allocator(self, WEIGHTS_C, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_c(self):
        # rule: c  (set_weights on SPY)
        if not (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 14, 'symbol': 'SPY'}, 'SPY')) > (40))):
            return
        self._alloc_c.rebalance(
            self.time.date(), tag="ir:c", on_flat=self._on_flat)
