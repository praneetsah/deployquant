...
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['enter', 'target', 'flat']}


class FlatSell(QCAlgorithm):
    """Flat Sell — ejected from the block editor. Runs the SAME
    ...
    def _rule_target(self):
        # rule: target  (managed_target on SPY)
        ...
        qty = int(self.portfolio['SPY'].quantity)
        if qty <= 0:
            return
        limit_px = ((self._bc.entry_price('SPY')) * (1.01))
        self._place_or_update_target('SPY', qty, limit_px, tag="ir:target")

    def _rule_flat(self):
        # rule: flat  (market_order on SPY)
        if not self._tomorrow_is_last_trading_day():
            return
        if not (self.portfolio['SPY'].invested):
            return
        qty = int(abs(self.portfolio['SPY'].quantity) * (1.0))
        if qty > 0:
            self.market_order('SPY', -1 * qty, tag="ir:flat")
