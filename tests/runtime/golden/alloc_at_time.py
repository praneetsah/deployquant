...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['t']}
WEIGHTS_T = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_t = Allocator(self, WEIGHTS_T, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_t(self):
        # rule: t  (set_weights on SPY)
        self._alloc_t.rebalance(
            self.time.date(), tag="ir:t", on_flat=self._on_flat)
