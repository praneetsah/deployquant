...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['g']}
WEIGHTS_G = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_g = Allocator(self, WEIGHTS_G, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_g(self):
        # rule: g  (set_weights on SPY)
        if not (self._guard_week.get('g') != self._week_key(self._current_day)):
            return
        self._guard_week['g'] = self._week_key(self._current_day)
        self._alloc_g.rebalance(
            self.time.date(), tag="ir:g", on_flat=self._on_flat)
