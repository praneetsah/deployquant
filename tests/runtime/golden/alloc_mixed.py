...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['rot'], 'TLT': ['hedge']}
...
class AllocShapes(QCAlgorithm):
    ...
    def _rule_hedge(self):
        # rule: hedge  (market_order on TLT)
        if not self._is_first_of_month():
            return
        if not (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 14, 'symbol': 'SPY'}, 'SPY')) < (45))):
            return
        ref_price = self._bc.price('TLT')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.1) / ref_price)
        if qty > 0:
            self.market_order('TLT', 1 * qty, tag="ir:hedge")
