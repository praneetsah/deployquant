...
class IndicatorGate(QCAlgorithm):
    ...
    def _rule_enter(self):
        ...
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'pct_rank', 'lookback': 60, 'min_obs': 30, 'of': {'ind': 'rsi', 'window': 14}}, 'SPY')) > (0.8))))):
            return
    ...
    def _rule_exit(self):
        ...
        if not ((not (self._cmp(lambda: (self._bc.indicator({'ind': 'pct_rank', 'lookback': 60, 'min_obs': 30, 'of': {'ind': 'rsi', 'window': 14}}, 'SPY')) > (0.8))))):
            return
