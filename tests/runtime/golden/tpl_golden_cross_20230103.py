...
PARAMS = {'FAST': 50, 'SLOW': 200, 'SIZE': 0.98}
SYMS = ['SPY']
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['cross-up-buy', 'cross-down-sell']}


class GoldenCross(QCAlgorithm):
    """Golden Cross — ejected from the block editor. Runs the SAME
    ...
    def _rule_cross_up_buy(self):
        # rule: cross-up-buy  (market_order on SPY)
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'sma', 'window': {'param': 'FAST'}}, 'SPY')) > (self._bc.indicator({'ind': 'sma', 'window': {'param': 'SLOW'}}, 'SPY')))))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (self.SIZE) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:cross-up-buy")

    def _rule_cross_down_sell(self):
        # rule: cross-down-sell  (liquidate on SPY)
        if not (((self.portfolio['SPY'].invested) and (self._cmp(lambda: (self._bc.indicator({'ind': 'sma', 'window': {'param': 'FAST'}}, 'SPY')) < (self._bc.indicator({'ind': 'sma', 'window': {'param': 'SLOW'}}, 'SPY')))))):
            return
        self._liquidate_all('SPY', "ir:cross-down-sell")
    ...
    def _at_open(self):
        ...
        try:
            self._rule_cross_up_buy()
        except NotReady:
            pass
        try:
            self._rule_cross_down_sell()
        except NotReady:
            pass
