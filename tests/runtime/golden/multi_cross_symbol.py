...
class CrossGate(QCAlgorithm):
    """Cross Gate — ejected from the block editor. Runs the SAME
    ...
    def _rule_enter(self):
        ...
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 14, 'symbol': 'QQQ'}, 'QQQ')) > (55))))):
            return
    ...
    def _rule_exit(self):
        ...
        if not (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 14, 'symbol': 'QQQ'}, 'QQQ')) < (45))):
            return
