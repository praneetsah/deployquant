...
PARAMS = {'WEEKLY_DOLLARS': 250, 'DIP_DD': -0.15, 'DIP_DOLLARS': 500, 'DD_WINDOW': 20}
SYMS = ['TQQQ']
UNIVERSE = ['TQQQ']
PRIMARY = 'TQQQ'
RULES_BY_SYMBOL = {'TQQQ': ['weekly-buy', 'dip-buy']}


class WeeklyDcaDipBuyer(QCAlgorithm):
    """Weekly DCA + Dip Buyer — ejected from the block editor. Runs the SAME
    ...
    def _rule_weekly_buy(self):
        # rule: weekly-buy  (market_order on TQQQ)
        if not self._is_first_of_week():
            return
        if not (self._guard_week.get('weekly-buy') != self._week_key(self._current_day)):
            return
        if not (self._cmp(lambda: (self.portfolio.cash) > (50))):
            return
        ref_price = self._bc.price('TQQQ')
        qty = int((self.WEEKLY_DOLLARS) / ref_price)
        if qty > 0:
            self.market_order('TQQQ', 1 * qty, tag="ir:weekly-buy")
            self._guard_week['weekly-buy'] = self._week_key(self._current_day)

    def _rule_dip_buy(self):
        # rule: dip-buy  (market_order on TQQQ)
        if not (self._guard_month.get('dip-buy') != (self._current_day.year, self._current_day.month)):
            return
        if not (((self._cmp(lambda: (self.portfolio.cash) > (150))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'drawdown', 'window': {'param': 'DD_WINDOW'}}, 'TQQQ')) <= (self.DIP_DD))))):
            return
        ref_price = self._bc.price('TQQQ')
        qty = int((self.DIP_DOLLARS) / ref_price)
        if qty > 0:
            self.market_order('TQQQ', 1 * qty, tag="ir:dip-buy")
            self._guard_month['dip-buy'] = (self._current_day.year, self._current_day.month)
