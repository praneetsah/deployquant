...
UNIVERSE = ['SPY', 'TLT']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['buy-spy'], 'TLT': ['buy-tlt']}


class TwoSleeves(QCAlgorithm):
    """Two Sleeves — ejected from the block editor. Runs the SAME
    ...
    def _rule_buy_spy(self):
        # rule: buy-spy  (market_order on SPY)
        if not self._is_first_of_week():
            return
        if not ((not (self.portfolio['SPY'].invested))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (self.FRAC) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:buy-spy")

    def _rule_buy_tlt(self):
        # rule: buy-tlt  (market_order on TLT)
        if not (self._guard_week.get('buy-tlt') != self._week_key(self._current_day)):
            return
        if not (self._cmp(lambda: (self._bc.indicator({'ind': 'sma', 'window': 10}, 'SPY')) < (self._bc.indicator({'ind': 'sma', 'window': 30}, 'SPY')))):
            return
        ref_price = self._bc.price('TLT')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.25) / ref_price)
        if qty > 0:
            self.market_order('TLT', 1 * qty, tag="ir:buy-tlt")
            self._guard_week['buy-tlt'] = self._week_key(self._current_day)
