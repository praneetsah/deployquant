...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['o']}
WEIGHTS_O = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_o = Allocator(self, WEIGHTS_O, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_o(self):
        # rule: o  (set_weights on SPY)
        self._alloc_o.rebalance(
            self.time.date(), tag="ir:o", on_flat=self._on_flat)
