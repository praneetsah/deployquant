...
class IndicatorGate(QCAlgorithm):
    ...
    def _rule_enter(self):
        ...
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'series_sma', 'window': 5, 'of': {'ind': 'rsi', 'window': 14}}, 'SPY')) > (50))))):
            return
    ...
    def _rule_exit(self):
        ...
        if not ((not (self._cmp(lambda: (self._bc.indicator({'ind': 'series_sma', 'window': 5, 'of': {'ind': 'rsi', 'window': 14}}, 'SPY')) > (50))))):
            return
