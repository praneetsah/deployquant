...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['daily-rotate', 'extra']}
WEIGHTS_DAILY_ROTATE = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': {'param': 'LOOKBACK'}}, 'order': 'top', 'n': 1}}


class EtfMomentumRotation(QCAlgorithm):
    """ETF Momentum Rotation — ejected from the block editor. Runs the SAME
    ...
    def initialize(self):
        ...
        self._alloc_daily_rotate = Allocator(self, WEIGHTS_DAILY_ROTATE, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_daily_rotate(self):
        # rule: daily-rotate  (set_weights on SPY)
        self._alloc_daily_rotate.rebalance(
            self.time.date(), tag="ir:daily-rotate", on_flat=self._on_flat)

    def _rule_extra(self):
        # rule: extra  (market_order on SPY)
        if not ((not (self.portfolio['SPY'].invested))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.1) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:extra")
