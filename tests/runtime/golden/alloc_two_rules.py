...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['morning', 'close']}
WEIGHTS_MORNING = {'equal': [{'asset': 'SPY'}, {'asset': 'TLT'}]}
WEIGHTS_CLOSE = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_morning = Allocator(self, WEIGHTS_MORNING, PARAMS, SYMS, ctx=self._bc)
        self._alloc_close = Allocator(self, WEIGHTS_CLOSE, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_morning(self):
        # rule: morning  (set_weights on SPY)
        if not self._is_first_of_week():
            return
        self._alloc_morning.rebalance(
            self.time.date(), tag="ir:morning", on_flat=self._on_flat)

    def _rule_close(self):
        # rule: close  (set_weights on SPY)
        self._alloc_close.rebalance(
            self.time.date(), tag="ir:close", on_flat=self._on_flat)
