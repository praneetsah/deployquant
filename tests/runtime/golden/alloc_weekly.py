...
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['w']}
WEIGHTS_W = {'best': {'of': [{'asset': 'SPY'}, {'asset': 'QQQ'}, {'asset': 'IWM'}, {'asset': 'TLT'}], 'by': {'metric': 'cum_return', 'window': 60}, 'order': 'top', 'n': 1}}
...
class AllocShapes(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self._alloc_w = Allocator(self, WEIGHTS_W, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_w(self):
        # rule: w  (set_weights on SPY)
        if not self._is_first_of_week():
            return
        self._alloc_w.rebalance(
            self.time.date(), tag="ir:w", on_flat=self._on_flat)
