...
UNIVERSE = ['AAA', 'BBB']
PRIMARY = 'AAA'
RULES_BY_SYMBOL = {'AAA': ['open-alloc', 'daily-rebalance']}
WEIGHTS_OPEN_ALLOC = {'equal': [{'asset': 'AAA'}]}
WEIGHTS_DAILY_REBALANCE = {'equal': [{'asset': 'AAA'}, {'asset': 'BBB'}]}


class SecondBlocks(QCAlgorithm):
    """Second Blocks — ejected from the block editor. Runs the SAME
    ...
    def initialize(self):
        ...
        self.set_cash(10000)
        for t in SYMS:
            self.add_equity(t, Resolution.SECOND)
        ...
        self._alloc_open_alloc = Allocator(self, WEIGHTS_OPEN_ALLOC, PARAMS, SYMS, ctx=self._bc)
        self._alloc_daily_rebalance = Allocator(self, WEIGHTS_DAILY_REBALANCE, PARAMS, SYMS, ctx=self._bc)
    ...
    def _rule_open_alloc(self):
        # rule: open-alloc  (set_weights on AAA)
        self._alloc_open_alloc.rebalance(
            self.time.date(), tag="ir:open-alloc", on_flat=self._on_flat)

    def _rule_daily_rebalance(self):
        # rule: daily-rebalance  (set_weights on AAA)
        self._alloc_daily_rebalance.rebalance(
            self.time.date(), tag="ir:daily-rebalance", on_flat=self._on_flat)
