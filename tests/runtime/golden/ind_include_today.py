...
class IndicatorGate(QCAlgorithm):
    ...
    def _rule_enter(self):
        ...
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 10, 'include_today': True}, 'SPY')) > (50))))):
            return
    ...
    def _rule_exit(self):
        ...
        if not ((not (self._cmp(lambda: (self._bc.indicator({'ind': 'rsi', 'window': 10, 'include_today': True}, 'SPY')) > (50))))):
            return
