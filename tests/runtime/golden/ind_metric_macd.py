...
UNIVERSE = ['SPY']
PRIMARY = 'SPY'
RULES_BY_SYMBOL = {'SPY': ['enter', 'exit']}


class IndicatorGate(QCAlgorithm):
    """Indicator Gate — ejected from the block editor. Runs the SAME
    ...
    def _rule_enter(self):
        # rule: enter  (market_order on SPY)
        if not ((((not (self.portfolio['SPY'].invested))) and (self._cmp(lambda: (((((self._bc.indicator({'ind': 'ema', 'window': 12}, 'SPY')) - (self._bc.indicator({'ind': 'ema', 'window': 26}, 'SPY')))) - (self._bc.indicator({'ind': 'series_ema', 'window': 9, 'of': {'sub': [{'ind': 'ema', 'window': 12}, {'ind': 'ema', 'window': 26}]}}, 'SPY')))) > (0))))):
            return
        ref_price = self._bc.price('SPY')
        qty = int(float(self.portfolio.total_portfolio_value) * (0.5) / ref_price)
        if qty > 0:
            self.market_order('SPY', 1 * qty, tag="ir:enter")

    def _rule_exit(self):
        # rule: exit  (liquidate on SPY)
        if not ((not (self._cmp(lambda: (((((self._bc.indicator({'ind': 'ema', 'window': 12}, 'SPY')) - (self._bc.indicator({'ind': 'ema', 'window': 26}, 'SPY')))) - (self._bc.indicator({'ind': 'series_ema', 'window': 9, 'of': {'sub': [{'ind': 'ema', 'window': 12}, {'ind': 'ema', 'window': 26}]}}, 'SPY')))) > (0))))):
            return
        self._liquidate_all('SPY', "ir:exit")
