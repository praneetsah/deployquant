...
def test_take_returns_the_days_fill_for_that_symbol():
    led = ExecutionLedger([_f(D, "TQQQ", 100, 50.0)], reconciled_from=D)
    got = led.take(D, "TQQQ", "weekly-entry")
    assert [(g.qty, g.price) for g in got] == [(100, 50.0)]
...
def test_take_consumes_so_a_second_call_gets_the_next_fill():
    ...
    led = ExecutionLedger([_f(D, "TQQQ", 60, 50.0, boid="o1"),
                           _f(D, "TQQQ", 40, 50.01, boid="o2")],
                          reconciled_from=D)
    assert [g.qty for g in led.take(D, "TQQQ", "r1")] == [60]
...
def test_days_before_reconciled_from_are_unknown():
    led = ExecutionLedger([], reconciled_from=date(2026, 1, 1))
    assert led.take(date(2025, 12, 31), "TQQQ", "r") is None
    assert led.take(date(2026, 1, 1), "TQQQ", "r") == []


def test_leftovers_reports_unclaimed_fills():
    led = ExecutionLedger([_f(D, "TQQQ", 100, 50.0), _f(D, "SPY", 5, 500.0)],
                          reconciled_from=D)
    led.take(D, "TQQQ", "r1")
...
def test_take_consumes_every_execution_of_one_order():
    ...
    led = ExecutionLedger([_f(D, "TQQQ", 60, 50.0, boid="o1"),
                           _f(D, "TQQQ", 40, 50.01, ms=34302000, boid="o1")],
                          reconciled_from=D)
    got = led.take(D, "TQQQ", "weekly-entry")
    assert [(g.qty, g.price) for g in got] == [(60, 50.0), (40, 50.01)]
...
def test_symbol_matching_is_case_insensitive():
    ...
    prevent."""
    led = ExecutionLedger([_f(D, "TQQQ", 100, 50.0)], reconciled_from=D)
    assert [g.qty for g in led.take(D, "tqqq", "r")] == [100]
    led2 = ExecutionLedger([_f(D, "tqqq", 100, 50.0)], reconciled_from=D)
    assert [g.qty for g in led2.take(D, "TQQQ", "r")] == [100]
...
def test_capped_ledger_keeps_batch_semantics_on_past_days():
    ...
    led = LiveCappedLedger(
        ExecutionLedger([_f(past, "REW", 20, 25.0)], reconciled_from=past),
        live_from=D)
...
def test_capped_ledger_applies_existing_rows_on_live_days():
    ...
    from dqengine.runtime.core.ledger import LiveCappedLedger
    led = LiveCappedLedger(
        ExecutionLedger([_f(D, "TQQQ", 100, 50.0)], reconciled_from=D),
        live_from=D)
...
def test_capped_ledger_delegates_attributes_and_leftovers():
    from dqengine.runtime.core.ledger import LiveCappedLedger
    inner = ExecutionLedger([_f(D, "TQQQ", 100, 50.0)], unknown={"spy"},
                            reconciled_from=D)
    led = LiveCappedLedger(inner, live_from=D)
    assert led.unknown == {"SPY"}
    assert led.reconciled_from == D
    assert [f.qty for f in led.leftovers()] == [100]
...
def test_presence_beats_unknown():
    """A poll hiccup puts TQQQ in the unknown set and HIDES yesterday's
    confirmed row, flapping the live basis. Rows are truth; unknown may
    only demote absence."""
    led = ExecutionLedger([_f(D, "TQQQ", 100, 50.0)], unknown={"TQQQ"},
                          reconciled_from=D)
    assert [(g.qty, g.price) for g in led.take(D, "TQQQ", "weekly-entry")] \
        == [(100, 50.0)]
...
def test_take_rule_fill_strict_tag_and_time_gate():
    rows = [_f(D, "TQQQ", -100, 51.0, ms=34496000, tag="tiered-target",
               boid="o1"),
            _f(D, "TQQQ", 100, 50.975, ms=34503000, tag=None, boid="o2")]
    ...
    assert [(g.qty, g.price) for g in got] == [(-100, 51.0)]
...
def test_take_rule_fill_shares_consumption_with_take():
    rows = [_f(D, "TQQQ", -100, 51.0, tag="tiered-target", boid="o1")]
    led = ExecutionLedger(rows, reconciled_from=D)
